...
const INDEX_KEY = 'boxbuilder-export-history-index';
const INDEX_PAGE_PREFIX = 'boxbuilder-export-history-index-page';
const ENTRY_PREFIX = 'boxbuilder-export-history-entry';
...
const buildSummary = (form: FormObject): string => `${form.width}×${form.depth}×${form.height}mm`;
...
export const listExportHistory = (): ExportHistoryListItem[] =>
  readIndexRows()
    .map(({ id, fileName, summary, exportedAt }) => ({ id, fileName, summary, exportedAt }))
...
export const recordExportHistory = (form: FormObject): ExportHistoryListItem | null => {
  try {
    ...
    rows.push({ id, fileName, summary, exportedAt, storageKeys });
    writeIndexRows(rows);

    return { id, fileName, summary, exportedAt };
  } catch {
    return null;
  }
};
