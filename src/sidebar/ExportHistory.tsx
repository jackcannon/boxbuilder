...
export const ExportHistory = ({ entries, selectedId, onSelect, onApply, onDelete }: Props) => {
  ...
  const formatLabel = (fileName: string, summary: string) => `${fileName} (${summary})`;
        ...
        <FormControl size="small" className="export-history-select">
          <Select
            ...
            renderValue={(value) => {
              ...
              return entry ? formatLabel(entry.fileName, entry.summary) : 'Select an export…';
            }}
          >
            {entries.map((entry) => (
              <MenuItem key={entry.id} value={entry.id}>
                {formatLabel(entry.fileName, entry.summary)}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
